...
export function signToken(claims) {
  ...
  const body = encode({
    ...claims,
    iat: now,
    exp: now + ttlSeconds(config.jwtTtl),
  });
...
export function authMiddleware(req, res, next) {
  ...
  if (!header.startsWith("Bearer "))
    return res
      .status(401)
      .json({ error: "AUTH_REQUIRED", message: "Anmeldung erforderlich." });
  try {
    ...
    return res
      ...
      .json({
        error: "INVALID_TOKEN",
        message: "Anmeldung ist ungueltig oder abgelaufen.",
      });
  }
}
  ...
  return (req, res, next) => {
    if (!req.user?.permissions?.includes(permission))
      return res
        ...
        .json({
          error: "FORBIDDEN",
          message: "Fuer diese Aktion fehlt die Berechtigung.",
        });
    return next();
  };
}
